...
dist10<-cbind(dist10, F, fr, Fra)
dist10

Total<-c("Total", NA, sum(f), NA, sum(fr), NA)


dist10<-rbind(dist10, Total)
dist10

rownames(dist10)<-seq(1, k+1, 1)
dist10

dist10<-data.frame(dist10)
dist10

###################Histograma#################

hist(data1$raisedhands, main="Histograma para variável RaisedHands", 
     xlab="Raised Hands", ylab="Freqüências", col="Pink")
