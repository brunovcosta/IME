...
h<-hist(data1$AnnouncementsView)
...
dist12<-cbind(dist12, F, fr, Fra)
dist12

Total<-c("Total", NA, sum(f), NA, sum(fr), NA)


dist12<-rbind(dist12, Total)
dist12

rownames(dist12)<-seq(1, k+1, 1)
dist12

dist12<-data.frame(dist12)
dist12

###################Histograma#################

hist(data1$AnnouncementsView, main="Histograma para variável AnnouncementsView", 
     xlab="Announcements View", ylab="Freqüências", col="yellow")
