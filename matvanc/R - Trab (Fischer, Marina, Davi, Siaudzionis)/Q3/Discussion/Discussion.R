...
dist13<-cbind(dist13, F, fr, Fra)
dist13

Total<-c("Total", NA, sum(f), NA, sum(fr), NA)


dist13<-rbind(dist13, Total)
dist13

rownames(dist13)<-seq(1, k+1, 1)
dist13

dist13<-data.frame(dist13)
dist13

###################Histograma#################

hist(data1$Discussion, main="Histograma para variável Discussion", 
     xlab="Discussion", ylab="Freqüências", col="green")
