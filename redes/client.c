...
int main(int argc, char *argv[]) {
	...
	if (argc != 4) {
		fprintf(stderr, "Utilize: Cliente <server_ip> <port> <file>\n");
		exit(1);
	}
	...
	//------------------------------------------
	send(csocket,argv[3],255,0);

	puts("Recebendo essa mensagem de volta oh:\n");
	int fileSize;
	recv(csocket,&fileSize,sizeof(int),0);
	printf("tamanho do arquivo eh: %d\n",fileSize);

	void *fileBuffer = malloc(fileSize);
	recv(csocket,fileBuffer,fileSize,0);
	char fileName[512] = "copia_";
	FILE *pFile = fopen(strcat(fileName,argv[3]),"wb");
	fwrite(fileBuffer,1,fileSize,pFile);
	//------------------------------------------
	fprintf(stdout, "\n");
	close(csocket);
	exit(0);
